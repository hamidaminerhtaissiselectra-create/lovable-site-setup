...
import { MapPin, Phone, ArrowRight, Shield, Award, Clock, Users, CheckCircle } from "lucide-react";
...
interface DepartmentHeroParallaxProps {
  department: DepartmentData;
  breadcrumbItems: { name: string; url: string }[];
}

const DepartmentHeroParallax = ({ department, breadcrumbItems }: DepartmentHeroParallaxProps) => {
  ...
  const priorityBadge = department.priority === 'primaire' 
    ? { label: "Zone Prioritaire", gradient: "from-green-500/20 to-emerald-500/20", textColor: "text-green-600", borderColor: "border-green-500/30" }
    : department.priority === 'secondaire'
    ? { label: "Zone Secondaire", gradient: "from-blue-500/20 to-cyan-500/20", textColor: "text-blue-600", borderColor: "border-blue-500/30" }
    : { label: "Zone Couverte", gradient: "from-slate-500/20 to-gray-500/20", textColor: "text-slate-600", borderColor: "border-slate-500/30" };

  // Stats spécifiques au département
  const stats = [
    { icon: Users, label: "Habitants", value: department.population },
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-5xl mx-auto text-center mt-8">
          ...
          >
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-primary/20 to-accent/20 text-primary font-semibold border border-primary/20 backdrop-blur-sm">
              <MapPin className="w-4 h-4 animate-pulse" />
              <span>Département {department.code} • {department.region}</span>
            </div>
            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r ${priorityBadge.gradient} ${priorityBadge.textColor} font-semibold border ${priorityBadge.borderColor} backdrop-blur-sm`}>
          ...
          >
            ...
            <span className="text-muted-foreground text-3xl md:text-4xl lg:text-5xl ml-2">({department.code})</span>
          </motion.h1>
...
export default DepartmentHeroParallax;
